...
- **Unauthenticated (P-AUTH-2):** any response indicating no/unknown/malformed token replaces the screen with the **"sign in required"** state — a full-screen message with a single action leading to `/sign-in`. Never partial data.
- **Unauthorized (P-AUTH-3b, 4, 6, 7):** any response indicating a denial for a revoked/absent membership, suspended tenant, or deactivated user replaces the screen with a full-screen access-denied state rendering the error envelope's message, with actions to retry and to sign out (present a different token). The UI does not invent sub-categories beyond what the envelope carries **[Node A seam: how the envelope distinguishes unauthenticated vs unauthorized]**; it maps exactly two UI classes (sign-in required vs access denied) onto the envelope's discriminator. Because membership is verified on every request (P-AUTH-3b), this state can appear mid-session on any screen; when it does, previously rendered tenant data is replaced, not left interactive.
...
List growth: P specifies complete lists with deterministic ordering and no pagination. If Node A's contract introduces pagination on any list, the UI presents an explicit "load more" continuation that preserves P's ordering; the UI must not re-sort client-side in a way that could contradict server ordering **[Node A seam: pagination, if any]**.
...
5. **Observe asynchronous results (P §4 step 6).** On the new timeline entry, the processing-state affordance (§5) progresses pending → published → processed (within the 30-second local bound). No additional task, timeline entry, or contact change appears from background processing (P §6.5).
...
- **Mid-session denial:** any step's request may return unauthorized (revocation, suspension, deactivation — P-AUTH-3b/4/6/7); the current screen is replaced by the access-denied state (§1.3); no write survives a prior-committed revocation (server-guaranteed; the UI simply renders the denial and never retries writes automatically).
...
- While a just-submitted interaction is not yet **processed**, the UI polls the inspection endpoint at a modest interval (implementation detail for Sprint 0002; the local acceptance bound is 30 seconds) and updates the indicator pending → published → processed without a page reload. Polling stops at processed or failed.
...
3. **Generated-title surfacing on the confirmation (§2.4):** confirm whether the submit response (or an immediate follow-up read) provides the created task's title/record so the confirmation shows server truth rather than a client-recomputed string. [Node A]
...
- P-AUTH-2..7 appear as user-visible behavior (§1.3, §3.1, §3.3); identity block always visible (§1.2, §6).
...
- Async observability designed as placement + affordance only; four states and failure_stage per P §6.4; endpoint marked as Node A seam; `seq`/`claim_id`/internal fields banned from the UI (§5, preamble).
